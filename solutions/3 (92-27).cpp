...
int main() {
    ...
    std::wofstream output_file(R"(C:\Projects\ML\RUCODE\D\predictions.txt)", std::ios::out);
...
#pragma omp parallel for
        for (int j = std::max(0, start_position - K);
            ...
            int lcp = 0; // longest commong prefix
            while (lcp < std::min(word.size(), train_words[j].size())
                && word[lcp] == train_words[j][lcp])
                lcp++;

            int lcs = 0; // longest commong substring
            for (int a = 0; a + lcp < word.size(); a++) {
                for (int b = 0; b + lcp < train_words[i].size(); b++) {
                    for (int c = 0; c + a < word.size() && c + b < train_words[i].size(); c++) {
                        if (word[a + c] == train_words[i][b + c]) {
                            lcs = std::max(lcs, c + 1);
                        } else {
                            break;
                        }
            ...
            int current_value = 
                -10 * lcp
                -5 * lcs
                + levenstein(word, train_words[i]);

            int stress_position = train_positions[j];
            if ((current_value < best_value || 
                (current_value <= best_value && rnd() % 3 == 0))
